...
from schemas.produto import ProdutoSchema, ProdutoListSchema, DeleteProdutoSchema
...
def get_produtos():
    ...
    for produto in produtos_query:
        produtos_list.append({"id": produto.id, "preco_venda": produto.preco_venda, "preco_custo": produto.preco_custo,
...
def create_produto(form: ProdutoSchema):
    ...
    if file and allowed_file(file.filename):
        ...
        try:
            ...
            session.add(produto)
            session.commit()
            return json.dumps(produto.to_dict()), 200
        except Exception as e:
...
@app.delete("/produto", tags=[produto_tag],
            responses={"200": DeleteProdutoSchema, "400": ErrorSchema, "500": ErrorSchema})
def delete_produto(form: DeleteProdutoSchema):
    session = Session()
    produto = session.query(Produto).filter(Produto.id == form.id).first()
    if produto:
